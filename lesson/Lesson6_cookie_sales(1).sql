...
-- the largest value that each girl was able to sell
SELECT MAX(sale_amount) 'Maximum Sale($)', first_name
FROM tbl_cookie_sales
GROUP BY first_name
ORDER BY 1 DESC;

--
SELECT MAX(sale_amount) 'Maximum Sale($)', sale_date
FROM tbl_cookie_sales
GROUP BY sale_date
ORDER BY 1 DESC;

--
SELECT SUM(sale_amount) 'Maximum Sale($)', sale_date
FROM tbl_cookie_sales
GROUP BY sale_date
ORDER BY 1 DESC;
...
-- When we specify column in COUNT, it won't count NULL values
...
SELECT first_name 'Name', sale_date, ROUND(AVG(sale_amount),2) 'Average Sale($)' -- SUM(sale_amount) 'Total Sales($)'
FROM tbl_cookie_sales
GROUP BY first_name, sale_date
ORDER BY first_name;
